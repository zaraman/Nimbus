...
contract NimbusReferralProgramMarketingV5 is Initializable, NimbusReferralProgramMarketingStorage {
    address public target;

    mapping(address => bool) public allowedOneTimeBonusReceivers;

    IInitial public initial;
    ...
    function _processVariableAirdrop(address user, uint256 potentialLevel, uint256 upgradeNonce, uint256 userVariableAirdropAmount, uint256 userNimbInBusdAirdropAmount, uint256 systemFee, bool dryRun) internal {
        ...
        if (userNimbInBusdAirdropAmount > 0) {
            uint256 nimbAmount = initial.getSystemTokenAmountForToken(address(paymentToken), userNimbInBusdAirdropAmount);
            TransferHelper.safeTransfer(initial.SYSTEM_TOKEN(), user, nimbAmount);
            emit AirdropNIMBReward(user, userNimbInBusdAirdropAmount, nimbAmount, potentialLevel);
            upgradeInfos[user][upgradeNonce].nimbRewardAmount = nimbAmount;
        }
    }
    ...
    function claimRewards(address user, uint256 userLevel, uint256 structureTurnover, string memory hash, uint256 userVariableAirdropAmount, uint256 userNimbInBusdAirdropAmount, uint256 systemFee) external onlyAllowedUpdaters {
        ...
        _processVariableAirdrop(user, potentialLevel, upgradeNonce, userVariableAirdropAmount, userNimbInBusdAirdropAmount, systemFee, dryRun);
    ...
    function updateInitial(address newInitial) external onlyOwner {
        require(AddressUpgradeable.isContract(newInitial), "newInitial is not a contract");
        initial = IInitial(newInitial);
    }
