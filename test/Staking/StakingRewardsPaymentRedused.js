...
describe("StakingRewardsPaymentRedused test", function () {
    ...
    it("Should add reduced reward and calculate correct rewards for special user HARDSTAKING_180", async function () {
        ...
        const reducedAmount = "10"; 
        const stakingPool = HARDSTAKING_180_20_MAINNET; 
        const userAddress = await impersonatedSigner2.getAddress();
        const nonce = 0;
    
        await StakingRewardsPaymentnew.addReducerRewardFactor(userAddress, nonce, reducedAmount, stakingPool);
        earnedByNonceWithReducedReward = await StakingRewardsPaymentnew.earnedByNonceWithReducedReward(userAddress,nonce,stakingPool)
        console.log(`earnedByNonceWithReducedReward: ${utils.formatEther(earnedByNonceWithReducedReward)} `)
        ...
        expect(userBalanceAfter).to.be.equal(earnedByNonceWithReducedReward);
        console.log(userBalanceAfter);

    });

    it("should add reduced reward and calculate correct rewards for special user HARDSTAKING_365", async function () {
        const impersonatedSigner2 = await ethers.getSigner("0xd0a111d4349aad1f8ce82e61d5c8c2beac6b2e92");
        ...
        const stakeTimestamp_365_2 = StakeNonceInfos_365_2.stakeTime;
        ...
        const rewardRate_365_2 = StakeNonceInfos_365_2.rewardRate;

        const calcRewards_365_2 = calculateRewards(stakeTimestamp_365_2, curTimestamp_365_2, rewardsTokenAmount_365_2, rewardRate_365_2);
        ...
        const reducedAmount = "10"; 
        const stakingPool = HARDSTAKING_365_60_MAINNET; 
        const userAddress = await impersonatedSigner2.getAddress();
        const nonce = 0;
        ...
        const userBalanceAfter = await BUSD.balanceOf(userAddress);
        expect(userBalanceAfter.sub(userBalanceBefore)).to.be.equal(earnedByNonceWithReducedReward);
    });
