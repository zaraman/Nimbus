...
const SLP_PAYMENT_TOKEN = process.env.SLP_PAYMENT_TOKEN || '';
...
async function main() {
  ...
  if (SLP_NIMB_TOKEN.length > 0 && NIMB_BNB_PAIR.length > 0 && NIMB_BNB_LPSTAKING.length > 0 && SLP_PAYMENT_TOKEN.length > 0 && SLP_NIMB_TOKEN.length > 0) {
    ...
    await smartLPNew.updateNimbTokenContract(SLP_NIMB_TOKEN, NIMB_BNB_PAIR, NIMB_BNB_LPSTAKING).then(async res=> await res.wait().then(console.log('Set NIMB token')));
    ...
    // await smartLPNew.updateUsePriceFeeds(false).then(async res=> await res.wait().then(console.log('PriceFeed disabled')));
  }
